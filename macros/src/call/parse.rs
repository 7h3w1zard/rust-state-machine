...
/// This object will collect all the information
/// we need to keep while parsing the callable functions.
#[derive(Debug)]
pub struct CallDef {
    /// This is the name of the pallet struct where the callable functions
    /// are implemented. We mostly assume it is `Pallet`.
...
impl CallDef {
    pub fn try_from(item: syn::Item) -> syn::Result<Self> {
        ...
        // Extract the name of the struct. We mostly assume it is `Pallet`,
        // but we can handle it when it isn't.
        let pallet_struct = match &*item_impl.self_ty {
            syn::Type::Path(tp) => tp.path.segment.first().unwrap().ident.clone(),
            _ => panic!("not supported tokens"),
        };

        // Here is where we will store all callable functions.
        let mut methods = vec![];
            ...
            if let syn::ImplItem::Fn(method) = item {
                ...
                match method.sig.inputs.iter().skip(1).next() {
                    Some(syn::FnArg::Typed(arg)) => {
                        // Here we specifically check that this argument is
                        // as we expect for `caller: T::AccountId`.
                        check_caller_arg(arg)?;
                    },
                    _ => {
                        let msg = "Invalid call, second argument must be `caller: T::AccountId`";
                        return Err(syn::Error::new(method.sig.span(), msg))
                    },
                }
